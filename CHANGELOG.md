...
- **Last values survive a restart or reload (local cache)** -- the retention above previously only worked while Home Assistant kept running; a restart or an integration reload recreated the coordinators empty, so every entity went `unavailable` until the next successful fetch (and, for BankID, until a manual re-scan). Each coordinator's last-known data is now persisted to a small on-disk cache and restored on startup, so entities come back showing their last values (marked `data_stale`) even when the session is already dead. The re-authentication prompt still appears -- the cache never hides that action is needed -- and completing it refreshes everything. The cache is removed when the integration is deleted.
...
### Fixed
- **Waste data failed on every reload/restart, blanking the whole integration** -- the waste coordinator (the first to run on setup) went straight to `flex/flexservices.aspx` without first visiting `start.aspx`. On a restored session the cookies are valid but the per-session server-side view state set up during login is gone, so the portal redirected to logout (302). Password logins silently recovered, but BankID could not, so the first fetch after any reload/restart failed immediately -- masking everything else and making BankID auth seem to "never stick". The waste fetch now visits `start.aspx` first, mirroring the consumption fetch and the login flow.
