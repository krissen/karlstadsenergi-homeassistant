...
2. **Your session has expired.** The portal uses ASP.NET session cookies with a short server-side lifetime.
   ...
   - **BankID users:** BankID sessions expire after only ~15 minutes and cannot be kept alive, so you will see a recurring re-authentication ("Reconfigure") prompt. Sensors keep showing their last values (marked `data_stale`) in the meantime. Go to Settings > Devices & Services > Karlstadsenergi > Reconfigure and sign in with BankID again to refresh. To escape the ~15-minute cycle for good, the Reconfigure dialog now lets you **switch to customer number & password** on the same screen (choose *Kundnummer & lösenord*) -- see the BankID section below.

3. **Orphaned entities.** A waste entity can end up "unavailable" in the registry for two reasons. First, **a service renamed before v0.5.0**: earlier versions keyed entities on the service *name*, so a portal rename (for example "Glas/Metall" -> "Glas- och metallförpackningar") orphaned the old entity. As of v0.5.0 entities are keyed on a stable service id and renames are migrated automatically, but a rename that happened *before* you updated leaves one stale entry. Second, **a mode switch**: the integration creates waste entities in one of two modes (detailed per-service-line or summary per-waste-type) depending on what data the Flex API returns; if the mode changes between restarts, the old set becomes "unavailable".
...
**Strong recommendation:** If at all possible, switch to customer number and password authentication instead. It re-authenticates silently and indefinitely and is far more reliable for long-term use. You no longer need to delete and re-add the integration to switch: at the next re-authentication prompt (or via **Reconfigure**), choose **Kundnummer & lösenord** and enter your customer number and password. See the [README](https://github.com/krissen/karlstadsenergi-homeassistant/blob/HEAD/README.md#authentication-methods) for details. You can set up a password at [Karlstadsenergi Password Reset](https://minasidor.karlstadsenergi.se/Customer/PasswordReset.aspx).
